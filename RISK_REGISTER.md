...
| R-04 | Planner unbounded request amplification (large CBOM → larger response) | MEDIUM | Medium | DoS cost amplification | `QTRUST_PLANNER_MAX_ASSETS` cap (verified live), rate limiter, API-key auth, `/docs`+`/openapi.json` disabled in production (TM/BP pass) | MITIGATED |
...
| R-09 | Server-side fetch of on-chain attacker-controlled `metadata_uri` (SSRF, TM-FE-01) | HIGH (exposed deployments) | Low | Internal service response disclosure via public page | **FIXED (PR #57):** strict CIDv0/CIDv1 validation + `redirect: "error"` in `fetchIpfsJson`; 8 regression tests | CLOSED |
